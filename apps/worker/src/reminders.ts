...
import { db, pushSubscription, reminder, task, user, userPrefs } from '@todolist/db';
import { sendEmail } from './email.js';
import { env } from './env.js';
import { pushEnabled, sendPush } from './push.js';

/** Deliver due reminders (email + web push, per the user's channel prefs), then mark them sent. */
export async function sendDueReminders() {
  ...
  const due = await db
    .select({
      ...
      // Nullable: users without a prefs row get the defaults (both on).
      notifyEmail: userPrefs.notifyEmail,
      notifyPush: userPrefs.notifyPush,
    })
    .from(reminder)
    .innerJoin(task, eq(task.id, reminder.taskId))
    .innerJoin(user, eq(user.id, reminder.userId))
    .leftJoin(userPrefs, eq(userPrefs.userId, reminder.userId))
    .where(and(lte(reminder.sendAt, now), isNull(reminder.sentAt)));

  for (const r of due) {
    const url = `${env.WEB_ORIGIN}/task/${r.taskId}`;
    if (r.notifyEmail ?? true) {
      try {
        await sendEmail({
          to: r.email,
    ...
    if ((r.notifyPush ?? true) && pushEnabled()) {
      const subs = await db
        .select()
