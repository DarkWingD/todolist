...
export function DoseSheet({
  ...
  const options = [
    ...presets,
    ...STARTERS.filter((s) => !presets.some((p) => p.medicine.toLowerCase() === s.medicine.toLowerCase())),
  ];
  const first = options[0];
  const [medicine, setMedicine] = useState(first?.medicine ?? '');
  const [amount, setAmount] = useState(first?.amount ?? '');
  const [interval, setInterval_] = useState(first?.intervalHours ? String(first.intervalHours) : '');
          ...
          {options.map((p) => (
            <button
              ...
              style={{
                fontSize: 'var(--fs-sm)',
                background:
                  !custom && medicine === p.medicine ? 'var(--color-accent-soft)' : 'var(--color-chip-bg)',
                color: !custom && medicine === p.medicine ? 'var(--color-accent)' : 'var(--color-text)',
              }}
            >
              {p.medicine}
            </button>
