...
export function CalScreen({
  ...
  function itemsFor(day: Date): CalItem[] {
    ...
    for (const t of tasks) {
      if (
        t.dueAt &&
        sameDay(startOfDay(new Date(t.dueAt as unknown as string)), day) &&
        !isHidden(t.assigneeId)
      ) {
        out.push({
          key: 't' + t.id,
          kind: 'task',
    ...
    return (
      ...
      >
        <span
          ...
          style={{ width: 4, background: it.color, minHeight: 28 }}
        />
      ...
      cells.push(
        ...
        >
          <span
            ...
            style={{
              fontSize: 'calc(10.5px * var(--text-scale))',
              fontWeight: 700,
              padding: isToday ? '1px 5px' : '0 2px',
              borderRadius: 999,
              marginBottom: 1,
              color: isToday
    ...
    for (let i = 0; i < 7; i++) {
      ...
      rows.push(
        <div
          ...
          style={{
            borderRadius: isWeekendCol(i) ? 12 : 0,
            background: isWeekendCol(i) ? 'var(--color-weekend, transparent)' : 'transparent',
            paddingLeft: isWeekendCol(i) ? 8 : 0,
            paddingRight: isWeekendCol(i) ? 8 : 0,
          }}
        >
      ...
      sections.push(
        <section key={i}>
          <h2
            ...
            style={{ fontSize: 'var(--fs-xs)', letterSpacing: '.09em' }}
          >
            {label}
          </h2>
          {items.map(evtRow)}
    ...
    return (
      <div className="flex-1 overflow-y-auto">
        {sections.length ? (
          ...
          <p className="mt-8 text-center text-muted" style={{ fontSize: 'var(--fs-base)' }}>
            Nothing coming up.
          </p>
        )}
      </div>
...
function CalSheet({
  ...
  const [end, setEnd] = useState(() =>
    toLocalInput(new Date(day.getTime() + 10 * 3600_000).toISOString()),
  );
